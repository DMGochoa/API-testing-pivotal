...
def validate_status_code(request, statuscode):
    ...
    """
    logging.info(
        f"""Validating the status code the response status should be {statuscode} and 
        is {request.response.status_code}""")
    assert request.response.status_code == int(statuscode)


@then(parsers.parse("the response body should contain the following data:\n{body_parameters}"))
